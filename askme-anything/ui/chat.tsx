...
export default function Chat() {
  ...
  async function sendToApi(msgs: ChatMessage[], systemPrompt: string) {
    ...
    try {
      ...
      // needed. One complete response (no streaming).
      const response = await airglow.llm.anthropic.messages({
      ...
      const rawContent: any[] = Array.isArray(response?.content) ? response.content : [];
      const blocks: MessageBlock[] = [];
      let searchQuery = '';
      for (const cb of rawContent) {
        if (cb.type === 'text' && cb.text?.trim()) {
          // Merge consecutive text blocks into one
          const lastBlock = blocks[blocks.length - 1];
          if (lastBlock && lastBlock.type === 'text') {
            lastBlock.text = (lastBlock.text || '') + ' ' + cb.text;
          } else {
            blocks.push({ type: 'text', text: cb.text });
          }
        } else if (cb.type === 'server_tool_use' && cb.name === 'web_search') {
          searchQuery = cb.input?.query || '';
          ...
          blocks.push({ type: 'search', search: { query: searchQuery, results } });
        }
      }

      const finalMsg: ChatMessage = { role: 'assistant', blocks, rawContent };
