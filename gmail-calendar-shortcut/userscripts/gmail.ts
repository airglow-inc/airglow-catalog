// Gmail Calendar Shortcut — extract meeting details from email threads, pre-fill calendar events
...
let autoCalendarConnectTried = false;
...
async function showWeek(anchor: string) {
  ...
  for (const mk of months) {
    if (!eventCache.has(mk)) {
      ...
      if (result === 'needsAuth') {
        const account = getLoggedInEmail() || undefined;
        const connectCalendar = async () => {
          try {
            await airglow.connectors.connect('googlecalendar', { account });
          } catch (e) {
        ...
        // Auto-start the OAuth popup once per page load; fall back to the button after a cancel/failure
        if (!autoCalendarConnectTried) {
          autoCalendarConnectTried = true;
          calDiv.innerHTML = `<div style="display:flex; align-items:center; justify-content:center; padding:20px; text-align:center;">
            <div style="font-size:13px; color:#5f6368;">Connecting Google Calendar…</div>
          </div>`;
          if (await connectCalendar()) return;
        }
        calDiv.innerHTML = `<div style="display:flex; align-items:center; justify-content:center; padding:20px; text-align:center;">
        ...
        calDiv.querySelector('#ag-cal-auth')?.addEventListener('click', connectCalendar);
        return;
      }
    }
  }
  ...
  btn.addEventListener('click', async (e) => {
    ...
    try {
      ...
      if (resp?.needsAuth) {
        ...
        // Chain the Calendar connection so the week view works without a second manual step
        try {
          const cal = await airglow.connectors.status('googlecalendar', { account: userEmail });
          if (!cal.connected) await airglow.connectors.connect('googlecalendar', { account: userEmail });
        } catch (e) {
          airglow.log.warn(`calendar connect after gmail failed: ${e}`);
        }
        stopDots();
        startDots('Generating');
